...
namespace base
{
inline void align_size(vk::DeviceSize& size,
...
public:
    ...
    vk::Format view_format{vk::Format()};
    vk::BufferView view;
    ...
    {
        buf = p_dev->dev.createBuffer(vk::BufferCreateInfo({},
                                                           size,
                                                           usage,
                                                           sharing_mode,
                                                           queue_family_count,
...
inline void allocate_and_bind_buffer_memory(Physical_device* p_phy_dev,
                                            Device* p_dev,
                                            vk::DeviceMemory& mem,
                                            uint32_t buf_count,
                                            Buffer** p_bufs,
                                            uint32_t override_alignment = 0)
{
    ...
    // this function does not unmap memory 
}
...
{
    assert((p_buffer->mem_prop_flags & vk::MemoryPropertyFlagBits::eDeviceLocal) == vk::MemoryPropertyFlagBits::eDeviceLocal);
    ...
    assert_success(p_dev->dev.resetFences(1, &fence));
